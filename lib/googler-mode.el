...
(defgroup googler-mode nil
  "Customization group for the googler-mode package for searching google and
   creating links."
  :group 'communication)

(defcustom googler-use-eww nil
  "If non-nil, googler-mode will use eww as the default web browser when
   opening links.")


(defcustom googler-number-results nil
  "If non-nil, googler-mode will return 10 results on a search. Otherwise, will
...
(defun googler-get-results (query &optional results-number)
  ...
  (let* ((googler-number-results (cond (results-number results-number)
         ...
         ;; If the search isn't exact there is a little message a at the beginning of the
         results
...
(defun googler-cumulative-build (results &optional carry)
  "Build lists of entry data based on RESULTS and bind global variables for
   navigation. CARRY is the starting point and can be used as offset for text at
   the beginning of the buffer before results."
  (if (eq results [])
      nil
    (let* ((built-list (googler-build-search-entry (elt results 0) carry))
           (begin-location (+ 2(cdr (cdr (assoc 'location-range built-list))))))
      ;; end-location)))
      (cons built-list (googler-cumulative-build (seq-drop results 1)
                                                 begin-location)))))


(defun googler-entity-list (entry-list assoc-symbol)
  "Takes built list of results as ENTRY-LIST and creates list based on an
   association with ASSOC-SYMBOL.
...
(defun googler-insert-fontified (string &optional font)
  "Insert text STRING with optional font face FONT. If font not specified or nil,
   fall back to a normal insert."
  (if font
...
(defun googler-render-entry (entry )
  "Uses an ENTRY from the built list of results to insert text into a buffer in the
   correct format."
  (progn
    (googler-insert-fontified (cdr (assoc 'title entry)) googler-title-font)
    (insert "\n\n")
    (googler-insert-fontified (cdr (assoc 'description entry))
                              googler-description-font)
    (insert "\n\n")))
...
(defun googler-results-buffer (query)
  ...
  (let* ((preamble (concat "Results for: " query "\n\n"))
         (built-list (googler-cumulative-build (googler-get-results query) (1+ (length
                                                                                preamble))))
         (buffer (get-buffer-create "*googler-results*"))
         (origin-buffer (buffer-name))
         (origin-region-active-p (region-active-p))
         (origin-region (if origin-region-active-p (cons (region-beginning)
                                                         (region-end)))))
    (with-current-buffer buffer
      (progn
        (let ((buffer-read-only nil))
          ...
          (setq googler-results-locations (mapcar 'car (googler-entity-list built-list
                                                                            'location-range)))
          (googler-mode)
...
(defun googler-search (prefix)
  ...
  (if (and prefix (= (car prefix) 4))
    ...
    (let ((googler-number-results (if current-prefix-arg current-prefix-arg
                                    googler-number-results)))
    ...
    (progn
      ...
      (insert
       (cond ((googler-origin-link-p 'org t) (googler-generate-link 'org title url))
             ...
             ((googler-origin-link-p 'markdown t) (googler-generate-link 'markdown title
                                                                         url))
...
(defun googler-insert-link-of-type (type &optional in-entry current-buffer)
  "Insert link of symbol TYPE into origin buffer. If optional IN-ENTRY specified,
   use that entry instead of the entry at point. If optional CURRENT-BUFFER
   specified, use current buffer instead of origin buffer."
  (interactive)
...
(defun googler-insert-link ()
  "Insert link for entry at point into origin buffer based on the mode active in
   origin buffer."
  (interactive)
...
(defun googler-insert-all-markdown-link ()
  ...
  (interactive)
  (mapcar (lambda (entry) (progn (googler-insert-link-of-type 'markdown entry)
                                 (insert " \n")))
          googler-entries-list))
...
(defun googler-insert-all-html-link ()
  ...
  (interactive)
  (mapcar (lambda (entry) (progn (googler-insert-link-of-type 'html entry)
...
(defun googler-origin-link-p (link-type &optional current-as-origin-buffer)
  "Return true if MODE in the category for LINK-TYPE. If
   CURRENT-AS-ORIGIN-BUFFER is non-nil, use current buffer instead of
   googler-origin-buffer."

  (member (if current-as-origin-buffer major-mode (with-current-buffer
                                                      googler-origin-buffer major-mode))
...
(defun googler-org-link ()
  ...
  (if (and (mark) (use-region-p))
      (let ((begin (region-beginning))
            (end (region-end)))
        (save-excursion
          (let ((link (googler-org-link-gen (buffer-substring (region-beginning)
                                                              (region-end)))))
            ;; (goto-char begin)
